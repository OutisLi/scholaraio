...
参数）。配置化运行时默认存储在 ``data/libraries/explore/<name>/``，并兼容
已有 ``data/explore/<name>/``，与主库完全隔离。

用法::

    from scholaraio.stores.explore import fetch_explore, build_explore_vectors, build_explore_topics
...
def _explore_root(cfg: Config | None = None) -> Path:
    if cfg is not None:
    ...
    return _DEFAULT_EXPLORE_DIR


def _explore_dir(name: str, cfg: Config | None = None) -> Path:
    return _explore_root(cfg) / name
...
def validate_explore_name(name: str) -> bool:
    ...
    configured explore root directory.
...
def build_explore_vectors(name: str, *, rebuild: bool = False, cfg: Config | None = None) -> int:
    ...
    Returns:
        本次新嵌入的论文数量。
    """
    from scholaraio.services.vectors import (
        _append_faiss_files,
        _embed_batch,
        _embed_provider,
        _embed_signature,
...
def build_explore_topics(
    ...
    *,
    ...
    from scholaraio.services.topics import build_topics
...
def _build_faiss_index(name: str, cfg: Config | None = None):
    ...
    from scholaraio.services.vectors import _build_faiss_from_db
...
def explore_vsearch(name: str, query: str, *, top_k: int = 10, cfg: Config | None = None) -> list[dict]:
    ...
    """
    from scholaraio.services.vectors import _embed_query_vector, _ensure_vector_search_ready, _vsearch_faiss
