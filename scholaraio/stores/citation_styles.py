...
(fresh default: data/libraries/citation_styles/<name>.py; legacy
data/citation_styles/<name>.py is auto-detected during migration).
...
3. Agent writes a Python formatter file to the configured citation styles
   directory following the interface above.
...
def styles_dir(cfg: Config) -> Path:
    """Return the configured citation styles cache directory."""
    citation_styles_dir = getattr(cfg, "citation_styles_dir", None)
    if citation_styles_dir is not None:
        return Path(citation_styles_dir)
...
def list_styles(cfg: Config) -> list[dict]:
    ...
    Args:
        cfg: Project configuration (used to resolve the citation styles path).
...
def get_formatter(name: str, cfg: Config) -> FormatterFn:
    ...
    Checks built-in styles first, then the configured dynamic style cache.
