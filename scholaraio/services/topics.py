...
用法：
    from scholaraio.services.topics import build_topics, get_topic_overview, get_topic_papers
...
from scholaraio.services.vectors import _embed_provider, _unpack
from scholaraio.stores.papers import best_citation as _best_cite
from scholaraio.stores.papers import read_meta as _read_meta

_log = logging.getLogger("scholaraio.topics")
...
def _ensure_numba_cache_dir() -> Path:
    """Give numba-backed BERTopic imports a writable cache in sandboxed runs."""
    cache_dir = Path(os.environ.get("NUMBA_CACHE_DIR") or Path(tempfile.gettempdir()) / "scholaraio-numba-cache")
    cache_dir.mkdir(parents=True, exist_ok=True)
    os.environ.setdefault("NUMBA_CACHE_DIR", str(cache_dir))
    return cache_dir


def _make_bertopic_embedder(cfg: Config | None = None):
    """Wrap the configured embedder in BERTopic's BaseEmbedder protocol."""
    _ensure_numba_cache_dir()

    import numpy as np
    from bertopic.backend._base import BaseEmbedder

    from scholaraio.services.vectors import QwenEmbedder

    class ScholarAIOBERTopicEmbedder(BaseEmbedder):
        def __init__(self):
            super().__init__()
            self._delegate = QwenEmbedder(cfg)

        def embed(self, documents, verbose: bool = False):
            return np.asarray(self._delegate.embed_documents(documents, verbose=verbose), dtype="float32")
    ...
    return ScholarAIOBERTopicEmbedder()
...
def _fit_bertopic(
    ...
    *,
    ...
    embedder = _make_bertopic_embedder(cfg)
...
def reduce_topics_to(
    ...
    if model.embedding_model is None:
        model.embedding_model = _make_bertopic_embedder(cfg)
